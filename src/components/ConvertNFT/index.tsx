import {
  ...
  const TestConvert = ({
    ...
    const onSubmit = async ({ ...submitValues }: CONVERTFORM) => {
      try {
        const { amount, width, count, start_has_ball } = submitValues;
        ...
        const unsignedPost = {
          ...
          width : width,
          count : count,
          start_has_ball : start_has_ball,
          userPubKey: pubkey.key,
          userSeq: addressinfo.getSequenceNumber(),
        };
  
        const unsignedUrl = `${walletServerAddr}/wallet/wallet-minting-unsigned`;
        ...
        const walletUrl = `${walletServerAddr}/wallet/wallet-minting`;
          ...
          {status === WalletStatus.WALLET_NOT_CONNECTED ? (
            ...
            >
              <div className="flex gap-2">
                <div className="min-w-[100px]">Pay Amount :</div>
                <input {...register("amount")} />
              </div>
              <div className="flex gap-2">
                <div className="min-w-[100px]">Width :</div>
              ...
              <div className="flex gap-2">
                <div className="min-w-[100px]">Start Has Ball :</div>
                <input {...register("start_has_ball")} />
              </div>
              <button
                type="submit"
