...
Swap 기능을 통해 CW20 토큰과 XPLA를 교환하거나, 혹은 CW20 토큰끼리도 쉽게 교환할 수 있습니다. 
유저들은 게임을 통해 얻은 CW20 토큰으로 쉽게 XPLA를 얻을 수 있고, XPLA를 게임에 필요한 CW20 토큰으로 교환할 수도 있습니다.

[Break The Bricks 게임](/#playgame)에서 사용하는 CW20 토큰<img src="/xpla-academy-dev/img/3-tutorial/3-deep-understand-xpla/5-convert/academy-token.svg" width="16" height="16" />과 XPLA<img src="/xpla-academy-dev/img/logo192.png" width="16" height="16" />를 교환하는 Swap Pool도 [Tool Page](/tool)에 있습니다. 
이번 단계에서는 CW20 토큰과 XPLA 간 Swap 기능을 어떻게 구현하는지 살펴보겠습니다.

:::info

이번 단계에서 Swap Pool을 만들기 위해 [Dezswap Contract](https://github.com/dezswap)를 사용하였습니다. 
...
여러분들은 새로 Contract를 개발하여 Swap Pool을 만드셔도 좋고, 예제처럼 Dezswap을 이용하셔도 좋습니다.
...
1. [Swap Pool 생성](/docs/tutorial/deep-understand-xpla/swap#create-swap-pool)
...
[토큰(CW20) 발행 및 전송하기 (Javascript)](/docs/tutorial/make-cw20/make-cw20-with-js) 단계에서 사용한 코드 중 name과 symbol만 `ACAD`로  수정하였습니다.
...
### 1. Swap Pool 생성 {#create-swap-pool}

예제에서는 Dezswap의 [dezswap_factory 컨트랙트](https://github.com/dezswap/dezswap-contracts/tree/main/contracts/dezswap_factory)를 실행시켜 Swap Pool을 생성할 것입니다.
...
위 코드를 실행하여 Testnet에서 Swap Pool을 생성하였습니다. 
...
[Event Logs](https://explorer.xpla.io/testnet/tx/f36baf53443365ea7bdb706f1bfd1b61f13ef7696e5bd155ed3996958b2364b9#EventLogs)를 확인해보면, 새로운 컨트랙트가 `instantiate`된 것을 알 수 있습니다. 
...
이때 [dezpswap_pair Contract는 CW20 토큰의 transfer 함수를 Execute](https://github.com/dezswap/dezswap-contracts/blob/main/contracts/dezswap_pair/src/contract.rs#L366)하여, 토큰을 공급하는 지갑으로부터 CW20 토큰을 가져옵니다.
...
이제 유동성 공급을 위해 dezswap_pair Contract의 [provide_liquidity 함수](https://github.com/dezswap/dezswap-contracts/tree/main/contracts/dezswap_pair#request-format)를 실행하겠습니다.
...
[3. 유동성 공급](/docs/tutorial/deep-understand-xpla/swap#provide-liquidity) 단계에서 Swap Pool에 총 10 XPLA와 10,000 ACAD를 공급하였습니다.
즉, Swap Pool의 토큰 개수의 곱은 항상 10 × 10000 = 100,000 만큼 유지되어야 합니다.
`ACAD` 토큰 1개를 넣은 경우, Swap Pool의 토큰 개수 곱은 (10 - XPLA_out)  × (10000 + 1) = 100,000이 되어야 합니다. 
...
여러분이 직접 구현한다면 유저들이 쉽게 Swap을 이용할 수 있도록, 따로 페이지를 제작하여 트랜잭션을 실행하는 API를 연결하는 것도 좋습니다. 

## 마무리

지금까지 Dezswap 컨트랙트를 이용하여 유저들이 쉽게 Swap할 수 있도록 하는 도와주는 방법에 대해 알아보았습니다.
